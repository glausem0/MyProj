...
public class Visitors implements MyParserVisitor{
	...
	private int searchLineProg(Object[] progArray, String elementLine){
		int returnI=0;
		for (int i=0; i < progArray.length; i++){
			if(progArray[i].toString().equals(elementLine)){
				returnI = i;
			}
		}
		return returnI;
	...
	public Object visit(ASTprog node, Object data) {
		...
		while(i<children){
			...
			case("label"):{
				...
				//label unique:
				branches.put("lineLabel"+node, searchLineProg(progArray, node.jjtGetChild(i).jjtAccept(this, data).toString()) );
		...
		default:
			...
			if(principalNode(node.jjtGetChild(i).toString())){
				pc +=1;
			}
			i+=1;
			reg.put("r15", pc);
